...
const renderers = {
   ...
   */
  blocks: {
    'unstyled': (children) => children.map(child => <span className="footnote-p">{child}</span>),
    'blockquote': (children) => <span className="footnote-blockquote" >{addBreaklines(children)}</span>,
    'header-one': (children, {keys}) => children.map((child, index) => <span className="footnote-h1" key={index} id={keys[index]}>{child}</span>),
    'header-two': (children, {keys}) => children.map((child, index) => <span className="footnote-h2" key={index} id={keys[index]}>{child}</span>),
    'header-three': (children, {keys}) => children.map((child, index) => <span className="footnote-h3" key={index} id={keys[index]}>{child}</span>),
    'header-four': (children, {keys}) => children.map((child, index) => <span className="footnote-h4" key={index} id={keys[index]}>{child}</span>),
    'header-five': (children, {keys}) => children.map((child, index) => <span className="footnote-h5" key={index} id={keys[index]}>{child}</span>),
    'header-six': (children, {keys}) => children.map((child, index) => <span className="footnote-h6" key={index} id={keys[index]}>{child}</span>),

    // You can also access the original keys of the blocks
    'code-block': (children, {keys}) => <span className="footnote-pre" key={keys[0]} >{addBreaklines(children)}</span>,
    // or depth for nested lists
    'unordered-list-item': (children, {depth, keys}) => <span className="footnote-ul" key={keys[keys.length - 1]} className={`ul-level-${depth}`}>{children.map((child, index) => <span className="footnote-li" key={index}>{child}</span>)}</span>,
    'ordered-list-item': (children, {depth, keys}) => <span className="footnote-ol" key={keys.join('|')} className={`ol-level-${depth}`}>{children.map((child, index) => <span className="footnote-li" key={keys[index]}>{child}</span>)}</span>,
